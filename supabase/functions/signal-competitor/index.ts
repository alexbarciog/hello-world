  ...
  try {
    const res = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'google/gemini-3-flash-preview',
        messages: [
          { role: 'system', content: `You decide if a LEAD's employer plausibly fits the user's ICP, using only the lead's headline and company name.\n\nUSER'S BUSINESS: "${businessContext || 'N/A'}"\nICP TARGET INDUSTRIES: ${icpIndustries.length ? icpIndustries.join(', ') : '(none specified)'}\nIDEAL LEAD DESCRIPTION:\n"""\n${idealLeadDescription || '(none provided)'}\n"""\n\nReturn matches_icp=false ONLY when the headline + company CLEARLY signal an irrelevant employer. Otherwise return true.\n\nRespond ONLY via the tool call.` },
...
async function insertContact(sb: any,p: any,uid: string,aid: string,ln: string,m: MatchResult,signal: string,spu: string|null,icp?: ICPFilters, manualApproval?: boolean, enrichedCompany?: EnrichedCompany | null, postExcerpt?: string | null, accountId?: string): Promise<'inserted' | 'duplicate' | 'rejected'>{
  ...
  const{data:ins,error}=await sb.from('contacts').insert({
    ...
    linkedin_profile_id:lpid,source_campaign_id:null,signal,signal_post_url:spu?(normalizePostUrl(spu)||spu):null,signal_post_excerpt:(postExcerpt||'').slice(0,500)||null,
      ...
      for (const engager of uniqueEngagers) {
        ...
        if (isHighPrecision) {
          ...
          if (seller.seller) {
            ...
            console.log(`[AGENCY_SELLER] 🚫 ${lpid}: ${(fp.headline||'').slice(0,80)} — matched: "${seller.matched}"`);
            continue;
          }
        ...
        const result = await insertContact(supabase, fp, user_id, agent_id, list_name, match, signal, engager.postUrl, icp, manual_approval, enrichedCompanyForInsert, engager.postText || '', account_id);
              ...
              for (const person of allFollowers) {
                ...
                if (isHighPrecision) {
                  ...
                  if (seller.seller) {
                    pipelineStats.company_icp_mismatch++;
                    captureRejected(fp, 'agency_seller');
                    console.log(`[AGENCY_SELLER] 🚫 follower ${lpid}: ${(fp.headline||'').slice(0,80)} — matched: "${seller.matched}"`);
                    continue;
                  }
                  const gate = await companyIcpGate(
                    fp, account_id, UNIPILE_API_KEY, UNIPILE_DSN,
                ...
                const result = await insertContact(supabase, fp, user_id, agent_id, list_name, match, `Follows ${companyName}`, url, icp, manual_approval, enrichedCompanyForInsert, undefined, account_id);
