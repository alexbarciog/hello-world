    ...
    if (error) {
      ...
      if (msg.includes("invalid login credentials")) {
        toast.error("Invalid email or password.", {
          description: "If you signed up with Google, please use the 'Continue with Google' button below.",
          duration: 6000,
        });
      } else {
      ...
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="w-full max-w-sm">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 py-2.5 rounded-lg border border-border bg-background hover:bg-muted/50 text-sm font-medium text-foreground transition-colors disabled:opacity-60">
            {googleLoading ? (
              <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
